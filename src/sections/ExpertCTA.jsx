import React from "react";
import { ArrowRight } from "lucide-react";

export default function ExpertCTA() {
  return (
    <section className="px-4 sm:px-6 py-8">
      <div className="relative max-w-7xl mx-auto overflow-hidden rounded-3xl min-h-[420px]">

        {/* Responsive Background */}
        <picture>
          {/* Desktop (≥768px) */}
          <source
            srcSet="./expert-hero-desktop.png"
            media="(min-width: 768px)"
          />

          {/* Mobile (<768px) */}
          <img
            src="./expert-hero-mobile.png"
            alt="Logistics container ship"
            className="absolute inset-0 w-full h-full object-cover"
          />
        </picture>

        {/* Overlay */}
        <div className="absolute inset-0 bg-gradient-to-r from-black/65 via-black/35 to-transparent" />

        {/* Content */}
        <div className="relative z-10 px-6 sm:px-10 py-14 sm:py-16 max-w-[560px]">
          <h2 className="uppercase font-extrabold text-white leading-tight text-[clamp(34px,5vw,64px)]">
            <span className="block">Talk to a</span>
            <span className="block">Logistics</span>
            <span className="block">Expert</span>
          </h2>

          <p className="mt-4 text-white/90 text-sm sm:text-base leading-relaxed">
            Get solutions that fit your business needs. Talk to a supply chain
            expert today.
          </p>

          <div className="mt-6">
            <a
              href="#contact"
              className="inline-flex items-center gap-3 bg-white text-[color:var(--brand-600,#6d4bd6)] rounded-full px-6 py-3 font-semibold shadow-lg hover:shadow-xl transition"
            >
              Contact Us <ArrowRight size={18} />
            </a>
          </div>
        </div>

      </div>
    </section>
  );
}
